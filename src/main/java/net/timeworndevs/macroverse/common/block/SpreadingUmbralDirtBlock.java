...
public abstract class SpreadingUmbralDirtBlock extends SnowyDirtBlock {

    protected SpreadingUmbralDirtBlock(Properties properties) {
        super(properties);
    }
    ...
    protected abstract MapCodec<? extends SpreadingUmbralDirtBlock> codec();
        ...
        if (!canBeGrass(blockState, serverLevel, blockPos)) {
            ...
            if (serverLevel.getMaxLocalRawBrightness(blockPos.above())>= 0) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (serverLevel.getBlockState(blockPos2).is(MacroverseBlocks.UMBRAL_SOIL) && canPropagate(blockState2, serverLevel, blockPos2)) {
                        serverLevel.setBlockAndUpdate(blockPos2, (BlockState) blockState2.setValue(SNOWY, isSnowySetting(serverLevel.getBlockState(blockPos2.above()))));
                    }
                }
            }
        }
